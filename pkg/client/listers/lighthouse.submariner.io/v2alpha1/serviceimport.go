...
type ServiceImportLister interface {
	...
	List(selector labels.Selector) (ret []*v2alpha1.ServiceImport, err error)
...
// List lists all ServiceImports in the indexer.
func (s *serviceImportLister) List(selector labels.Selector) (ret []*v2alpha1.ServiceImport, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2alpha1.ServiceImport))
	})
	return ret, err
}
...
type ServiceImportNamespaceLister interface {
	...
	Get(name string) (*v2alpha1.ServiceImport, error)
...
// List lists all ServiceImports in the indexer for a given namespace.
func (s serviceImportNamespaceLister) List(selector labels.Selector) (ret []*v2alpha1.ServiceImport, err error) {
...
// Get retrieves the ServiceImport from the indexer for a given namespace and name.
func (s serviceImportNamespaceLister) Get(name string) (*v2alpha1.ServiceImport, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2alpha1.Resource("serviceimport"), name)
	}
	return obj.(*v2alpha1.ServiceImport), nil
}
